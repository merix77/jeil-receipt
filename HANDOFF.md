...
로컬에서 전 기능 동작 확인 완료. git 로컬 커밋 3개 (`da47139`까지), **GitHub 저장소는 아직 없음**. 배포 전 단계.
...
| Method | Path | 설명 |
|---|---|---|
| POST | `/receipts` | 이미지 업로드 → OCR → 항목 저장 |
| PATCH | `/receipts/:id/items` | 사용자 수정 반영 |
| POST | `/receipts/:id/confirm` | 구글시트(매입) 반영 |
...
**CLAUDE.md 미반영**: 아래 3개는 CLAUDE.md의 API 흐름 항목에 없음 — `GET /receipts/:id/image`, `/hygiene/check`, `/hygiene/education`.

## 화면 (frontend/src/pages/)

- **HomeScreen** — 📷 거래명세서 촬영하기(히어로) / 🔍 거래명세서 조회하기 / 위생점검·위생교육 카드 2개 / 오늘 기록 건수 / 위생 시트 바로가기 링크
- **ReviewScreen** — 항목별 수정, "?" 남으면 기록 차단, "장부에 기록하기"
- **HistoryScreen** — 월별 조회, 행 펼치면 원본 사진 + 항목 표 + 미반영 건 재전송, 반영 완료는 도장(印)
...
4. **위생 문서 백필**: 며칠 밀려도 버튼 한 번에 누락분 전부 등록. 점검표는 표시 없는 날만 채움(직접 넣은 △/X는 보존), 교육은 마지막 등록일 다음날~오늘 (최대 31일).
5. **고정값** (`backend/src/config/business.js`): 점검자 노영곤 / 장소 제일축산 / 교육시간 30분~1시간 / 대상 전직원 / 참석 2명. 교육 주제·내용은 샘플 10종 순환(`services/educationService.js`).
6. **중복 등록 차단**: 추출된 모든 항목의 (거래일자 + 이력번호)가 이미 DB에 있으면 409로 거부.
7. **위생점검/교육 조회는 앱에 없음** — 구글시트 바로가기 링크로 처리(하이브리드 A안).

## 함정 (반드시 알고 있을 것)

- **`.env` 파일은 도구로 읽기/쓰기 불가** (권한 차단). 값 변경이 필요하면 **사용자에게 직접 수정 요청**해야 함.
- **시트 ID 오타 주의**: 매입 시트 ID에 대문자 `I` / 소문자 `l` 혼동으로 404를 겪었음. 현재 `.env`는 정상값(`...YYNoM0iKflR-...`)이지만 **CLAUDE.md의 표에는 아직 오타(`...iKfIR-...`)가 남아있음** — 복붙 금지.
...
4. 판매(영업자간 거래내역서) 시트 미구현 — `SHEET_ID_SALES`는 .env에만 있고 코드에서 안 씀
